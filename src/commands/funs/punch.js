const {
  ...
  module.exports = {
    data: new SlashCommandBuilder()
      .setName("soco")
      .setDescription("Dar um soco em um usuário")
    ...
    async execute(interaction, client, args) {
      ...
      const embed = new EmbedBuilder()
        .setDescription(
          `**${interaction.user} Deu um soco em ${user}.**`
        )
        .setColor("Random")
        .setTimestamp()
        .setImage(image)
      ...
      const embed2 = new EmbedBuilder()
        .setDescription(
          `**${user} Retribuiu o soco de ${interaction.user}.**`
        )
        .setColor("Random")
        .setTimestamp()
        .setImage(image)
      ...
      await interaction.reply({ embeds: [embed], components: [button] }).then(() => {
        const filter = (i) =>
          i.customId === "punch-button" && i.user.id === user.id;
